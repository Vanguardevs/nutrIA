...
const MessageBubble = React.memo(({ message, isUser, index, timestamp }: MessageBubbleProps) => {
  ...
  return (
    <View style={{ flexDirection: isUser ? "row-reverse" : "row", alignItems: "flex-end", marginVertical: 4 }}>
      ...
      >
        <Text style={[styles.messageText, { color: isUser ? "#FFFFFF" : "#1C1C1E" }]}>{message}</Text>
        <Text
          style={{
            fontSize: 11,
            color: isUser ? "#E0E0E0" : "#888",
            marginTop: 4,
...
type TypingIndicatorProps = { visible: boolean };
const TypingIndicator = React.memo(({ visible }: TypingIndicatorProps) => {
  const fadeAnim = useRef(new Animated.Value(0)).current;
...
type QuestionExamplesProps = { onQuestionPress: (text: string) => void };
const QuestionExamples = React.memo(({ onQuestionPress }: QuestionExamplesProps) => {
  ...
  const handlePress = (example: { description: string }) => {
        ...
        {examples.map((example) => (
          ...
          >
            <View style={[styles.exampleIconContainer, { backgroundColor: example.iconBg }]}>
              <Ionicons name={example.icon as any} size={isSmallScreen ? 14 : 16} color={example.iconColor} />
            </View>
            <Text style={[styles.exampleTitle, { fontSize: isSmallScreen ? 10 : 11 }]}>{example.title}</Text>
            <Text style={[styles.exampleDescription, { fontSize: isSmallScreen ? 8 : 9 }]}>
              {example.description}
            </Text>
          </TouchableOpacity>
        ))}
...
const DeleteConfirmationModal = React.memo(({ visible, onConfirm, onCancel }: DeleteConfirmationModalProps) => {
...
export default function Home({ navigation }: { navigation: any }) {
  ...
  useEffect(() => {
    (global as any).clearMessagesFunction = handleClearMessages;
    return () => {
      (global as any).clearMessagesFunction = null;
    };
  }, [handleClearMessages]);
  ...
  const sendMessageToAPI = useCallback(
    async (messageText: string) => {
      try {
        ...
        const response = await axios.post(API_URLS.QUESTION as string, payloadFinal, {
        ...
        let respostaLimpa: string = response.data.message.resposta;
  ...
  const renderMessage = useCallback(
    ({ item, index }: { item: Message; index: number }) => (
      <MessageBubble message={item.message} isUser={item.isUser} index={index} timestamp={item.timestamp} />
    ),
    [],
  );
  ...
  useEffect(() => {
    // Carregar mensagens do backend/Firebase se necessário
  }, []);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        {messages.length === 0 && <QuestionExamples onQuestionPress={handleQuestionPress} />}
        ...
        <FlatList
          ...
          ListEmptyComponent={() => (
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubText}>Use os exemplos acima ou digite sua pergunta</Text>
            </View>
          )}
        />
          ...
          <TouchableWithoutFeedback onPress={() => setShowSuggestions(false)}>
            <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.3)", justifyContent: "flex-end" }}>
              <TouchableWithoutFeedback>
                ...
                >
                  <Text style={{ fontSize: 18, fontWeight: "bold", marginBottom: 12, color: "#2E8331" }}>
      ...
      <DeleteConfirmationModal visible={showDeleteModal} onConfirm={confirmDelete} onCancel={cancelDelete} />
    </SafeAreaView>
  );
}
