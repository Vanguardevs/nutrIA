...
export default function ResumoSemanal() {
  const [dadosSemana, setDadosSemana] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    const exemploDados = semana.map((date) => ({
      ...
      calorias: Math.floor(Math.random() * 500) + 1500,
      valorEnergetico: Math.floor(Math.random() * 100) + 300,
      comidos: Math.floor(Math.random() * 10) + 5,
      naoComidos: Math.floor(Math.random() * 5),
    }));
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 34, fontWeight: "bold", marginBottom: 20, alignSelf: "center" },
  diaContainer: { marginBottom: 15, padding: 15, backgroundColor: "#e0f2f1", borderRadius: 8 },
  diaNome: { fontWeight: "bold", fontSize: 22, marginBottom: 8 },
  totalContainer: {
    marginTop: 25,
    padding: 20,
  ...
  totalTitle: { fontSize: 28, fontWeight: "bold", marginBottom: 10 },
});
